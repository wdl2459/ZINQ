...
ZINQ_tests <- function(formula.logistic, formula.quantile, C, y_CorD="C", data, taus=c(0.1, 0.25, 0.5, 0.75, 0.9), seed=2020){
  ...
  #   warning("There are no positive outcomes, NA returned.")
  #   return(pvalue=NA)
  # }
  #
  # # whether the C of interest has non-zero variance in the positive subset
  # if (any( apply(as.matrix(data[b==1, C], ncol=length(C)), 2, sd, na.rm=T) == 0)){
  #   warning("Some feature of interest has non-zero variance in the positive subset, NA returned.")
  #   return(pvalue=NA)
  # }
